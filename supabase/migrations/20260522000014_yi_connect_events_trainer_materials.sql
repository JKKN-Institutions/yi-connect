-- ═══════════════════════════════════════════════════════════════════════
-- Migration: Events Trainer Materials lifted to yi_connect.*
-- Phase A Batch 10b
--
-- Lifts public.* event-trainer-materials tables, enums, indexes, RLS,
-- triggers, and functions into yi_connect.* with these transformations:
--   - All yi-connect tables → yi_connect schema
--   - chapters FKs → yi.chapters (no chapter refs in this migration)
--   - ENUMs → yi_connect.* schema
--   - Functions → yi_connect.* schema
--   - Triggers reference yi_connect.update_updated_at_column()
--   - GENERATED ALWAYS columns kept (uses IMMUTABLE expressions only)
--
-- Source: 20251127000001_events_trainer_materials.sql (703 lines)
-- Dependencies: yi_connect.events, yi_connect.members, yi_connect.event_rsvps,
--               yi_connect.trainer_profiles (created in 20260522000013)
-- ═══════════════════════════════════════════════════════════════════════
...
CREATE TYPE yi_connect.trainer_assignment_status AS ENUM (
...
CREATE TYPE yi_connect.material_approval_status AS ENUM (
...
-- Service Event Type
CREATE TYPE yi_connect.service_event_type AS ENUM (
  'masoom',
  'thalir',
  'yuva',
  'road_safety',
...
-- Add 'school_services' to yi_connect.event_category if not exists
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_enum WHERE enumlabel = 'school_services' AND enumtypid = 'yi_connect.event_category'::regtype) THEN
    ALTER TYPE yi_connect.event_category ADD VALUE 'school_services';
  END IF;
END
$$;
...
-- Add service event fields to yi_connect.events table
ALTER TABLE yi_connect.events
ADD COLUMN IF NOT EXISTS is_service_event BOOLEAN DEFAULT false,
ADD COLUMN IF NOT EXISTS service_type yi_connect.service_event_type,
...
ON yi_connect.events(is_service_event, service_type)
WHERE is_service_event = true;

CREATE INDEX IF NOT EXISTS idx_events_stakeholder
ON yi_connect.events(stakeholder_type, stakeholder_id)
WHERE stakeholder_id IS NOT NULL;

COMMENT ON COLUMN yi_connect.events.is_service_event IS 'True for Yi Service Events (Masoom, Thalir, etc.) requiring trainers';
COMMENT ON COLUMN yi_connect.events.service_type IS 'Type of service session for service events';
COMMENT ON COLUMN yi_connect.events.expected_students IS 'Expected number of students/participants for service events';
COMMENT ON COLUMN yi_connect.events.trainers_needed IS 'Auto-calculated: 1 trainer per 60 students';

-- ============================================================================
-- EVENT TRAINER ASSIGNMENTS TABLE
-- ============================================================================

CREATE TABLE IF NOT EXISTS yi_connect.event_trainer_assignments (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  event_id UUID NOT NULL REFERENCES yi_connect.events(id) ON DELETE CASCADE,
  trainer_profile_id UUID NOT NULL REFERENCES yi_connect.trainer_profiles(id) ON DELETE CASCADE,

  -- Assignment Details
  status yi_connect.trainer_assignment_status NOT NULL DEFAULT 'recommended',
  ...
  confirmed_by UUID REFERENCES yi_connect.members(id),
...
-- Indexes
CREATE INDEX idx_event_trainer_event ON yi_connect.event_trainer_assignments(event_id);
CREATE INDEX idx_event_trainer_trainer ON yi_connect.event_trainer_assignments(trainer_profile_id);
CREATE INDEX idx_event_trainer_status ON yi_connect.event_trainer_assignments(status);
CREATE INDEX idx_event_trainer_pending ON yi_connect.event_trainer_assignments(response_deadline)
  WHERE status = 'invited';
CREATE INDEX idx_event_trainer_confirmed ON yi_connect.event_trainer_assignments(event_id, status)
  WHERE status IN ('accepted', 'confirmed', 'completed');

COMMENT ON TABLE yi_connect.event_trainer_assignments IS 'Trainer assignments for service events with AI-based scoring';

-- ============================================================================
-- EVENT MATERIALS TABLE (Multi-version with approval workflow)
-- ============================================================================

CREATE TABLE IF NOT EXISTS yi_connect.event_materials (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  event_id UUID NOT NULL REFERENCES yi_connect.events(id) ON DELETE CASCADE,
  trainer_assignment_id UUID REFERENCES yi_connect.event_trainer_assignments(id) ON DELETE SET NULL,
  ...
  parent_material_id UUID REFERENCES yi_connect.event_materials(id) ON DELETE SET NULL,
  version_notes TEXT,

  -- Approval Workflow
  status yi_connect.material_approval_status NOT NULL DEFAULT 'draft',
  submitted_at TIMESTAMPTZ,
  submitted_by UUID REFERENCES yi_connect.members(id),
  reviewed_at TIMESTAMPTZ,
  reviewed_by UUID REFERENCES yi_connect.members(id),
  review_notes TEXT,
  ...
  -- Upload tracking
  uploaded_by UUID NOT NULL REFERENCES yi_connect.members(id),
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
);

-- Indexes
CREATE INDEX idx_event_materials_event ON yi_connect.event_materials(event_id);
CREATE INDEX idx_event_materials_trainer ON yi_connect.event_materials(trainer_assignment_id);
CREATE INDEX idx_event_materials_status ON yi_connect.event_materials(status);
CREATE INDEX idx_event_materials_current ON yi_connect.event_materials(event_id, is_current_version)
  WHERE is_current_version = true;
CREATE INDEX idx_event_materials_templates ON yi_connect.event_materials(material_type, is_template)
  WHERE is_template = true;
CREATE INDEX idx_event_materials_approved ON yi_connect.event_materials(event_id, status)
  WHERE status = 'approved';

COMMENT ON TABLE yi_connect.event_materials IS 'Training materials with multi-version support and Chair/Vertical Chair approval workflow';

-- ============================================================================
-- EVENT SESSION REPORTS TABLE (Post-session reporting)
-- ============================================================================

CREATE TABLE IF NOT EXISTS yi_connect.event_session_reports (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  ...
  -- Submission
  submitted_by UUID NOT NULL REFERENCES yi_connect.members(id),
  submitted_at TIMESTAMPTZ DEFAULT now(),
  verified_by UUID REFERENCES yi_connect.members(id),
...
-- Indexes
CREATE INDEX idx_session_reports_event ON yi_connect.event_session_reports(event_id);
CREATE INDEX idx_session_reports_trainer ON yi_connect.event_session_reports(trainer_assignment_id);
CREATE INDEX idx_session_reports_followup ON yi_connect.event_session_reports(follow_up_date)
  WHERE follow_up_required = true AND follow_up_completed = false;
CREATE INDEX idx_session_reports_submitted ON yi_connect.event_session_reports(submitted_at DESC);

COMMENT ON TABLE yi_connect.event_session_reports IS 'Post-session reports for service events with attendance, impact metrics, and feedback';

-- ============================================================================
-- EVENT CARPOOL MATCHES TABLE
-- ============================================================================

CREATE TABLE IF NOT EXISTS yi_connect.event_carpool_matches (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  event_id UUID NOT NULL REFERENCES yi_connect.events(id) ON DELETE CASCADE,

  -- Driver
  driver_rsvp_id UUID NOT NULL REFERENCES yi_connect.event_rsvps(id) ON DELETE CASCADE,
  driver_member_id UUID NOT NULL REFERENCES yi_connect.members(id),

  -- Passenger
  passenger_rsvp_id UUID NOT NULL REFERENCES yi_connect.event_rsvps(id) ON DELETE CASCADE,
  passenger_member_id UUID NOT NULL REFERENCES yi_connect.members(id),
...
-- Indexes
CREATE INDEX idx_carpool_event ON yi_connect.event_carpool_matches(event_id);
CREATE INDEX idx_carpool_driver ON yi_connect.event_carpool_matches(driver_rsvp_id);
CREATE INDEX idx_carpool_passenger ON yi_connect.event_carpool_matches(passenger_rsvp_id);
CREATE INDEX idx_carpool_status ON yi_connect.event_carpool_matches(match_status);

COMMENT ON TABLE yi_connect.event_carpool_matches IS 'Carpool matching between members offering and needing rides for events';

-- ============================================================================
-- ADD CARPOOL FIELDS TO EVENT_RSVPS (if not exist)
-- ============================================================================

ALTER TABLE yi_connect.event_rsvps
...
CREATE OR REPLACE FUNCTION yi_connect.calculate_event_trainer_score(
...
BEGIN
  ...
  FROM yi_connect.trainer_profiles
  ...
  -- NOTE: Source bug — members has no engagement_score column
  -- (it lives on yi_connect.engagement_metrics). We pull from
  -- engagement_metrics by member_id; if no row, default to 8 points.
  DECLARE
    v_eng_score INTEGER;
  BEGIN
    SELECT engagement_score INTO v_eng_score
    FROM yi_connect.engagement_metrics
    WHERE member_id = v_trainer.member_id
    LIMIT 1;

    IF v_eng_score IS NOT NULL THEN
      IF v_eng_score >= 80 THEN
        v_engagement_score := 15;
      ELSIF v_eng_score >= 60 THEN
        v_engagement_score := 10;
      ELSIF v_eng_score >= 40 THEN
...
COMMENT ON FUNCTION yi_connect.calculate_event_trainer_score IS 'Calculate trainer match score for event assignment (100 points max)';

-- ============================================================================
-- UPDATE TRIGGERS
-- ============================================================================

-- Apply trigger to new tables (uses yi_connect.update_updated_at_column from foundation)
DROP TRIGGER IF EXISTS update_event_trainer_assignments_updated_at ON yi_connect.event_trainer_assignments;
CREATE TRIGGER update_event_trainer_assignments_updated_at
  BEFORE UPDATE ON yi_connect.event_trainer_assignments
  FOR EACH ROW EXECUTE FUNCTION yi_connect.update_updated_at_column();

DROP TRIGGER IF EXISTS update_event_materials_updated_at ON yi_connect.event_materials;
CREATE TRIGGER update_event_materials_updated_at
  BEFORE UPDATE ON yi_connect.event_materials
  FOR EACH ROW EXECUTE FUNCTION yi_connect.update_updated_at_column();

DROP TRIGGER IF EXISTS update_event_session_reports_updated_at ON yi_connect.event_session_reports;
CREATE TRIGGER update_event_session_reports_updated_at
  BEFORE UPDATE ON yi_connect.event_session_reports
  FOR EACH ROW EXECUTE FUNCTION yi_connect.update_updated_at_column();

DROP TRIGGER IF EXISTS update_event_carpool_matches_updated_at ON yi_connect.event_carpool_matches;
CREATE TRIGGER update_event_carpool_matches_updated_at
  BEFORE UPDATE ON yi_connect.event_carpool_matches
  FOR EACH ROW EXECUTE FUNCTION yi_connect.update_updated_at_column();
...
CREATE OR REPLACE FUNCTION yi_connect.update_trainer_stats_on_completion()
RETURNS TRIGGER AS $$
BEGIN
  -- Only trigger when status changes to 'completed'
  IF NEW.status = 'completed' AND (OLD.status IS NULL OR OLD.status != 'completed') THEN
    -- Update trainer profile stats
    UPDATE yi_connect.trainer_profiles
    SET
...
DROP TRIGGER IF EXISTS trigger_update_trainer_stats ON yi_connect.event_trainer_assignments;
CREATE TRIGGER trigger_update_trainer_stats
  AFTER INSERT OR UPDATE ON yi_connect.event_trainer_assignments
  FOR EACH ROW EXECUTE FUNCTION yi_connect.update_trainer_stats_on_completion();

-- ============================================================================
-- ROW LEVEL SECURITY
-- ============================================================================

ALTER TABLE yi_connect.event_trainer_assignments ENABLE ROW LEVEL SECURITY;
ALTER TABLE yi_connect.event_materials ENABLE ROW LEVEL SECURITY;
ALTER TABLE yi_connect.event_session_reports ENABLE ROW LEVEL SECURITY;
ALTER TABLE yi_connect.event_carpool_matches ENABLE ROW LEVEL SECURITY;

-- Event Trainer Assignments Policies
CREATE POLICY "Users can view trainer assignments for events they can see"
  ON yi_connect.event_trainer_assignments FOR SELECT
  TO authenticated
  USING (
    EXISTS (
...
CREATE POLICY "Event organizers can manage trainer assignments"
  ON yi_connect.event_trainer_assignments FOR ALL
  TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM yi_connect.events e
...
CREATE POLICY "Trainers can view their own assignments"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM yi_connect.trainer_profiles tp
...
CREATE POLICY "Trainers can respond to their assignments"
  ON yi_connect.event_trainer_assignments FOR UPDATE
  TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM yi_connect.trainer_profiles tp
...
CREATE POLICY "Users can view approved materials for events they can see"
  ON yi_connect.event_materials FOR SELECT
  TO authenticated
  USING (
...
CREATE POLICY "Trainers can manage their own materials"
  ON yi_connect.event_materials FOR ALL
  TO authenticated
  USING (uploaded_by = auth.uid());

CREATE POLICY "Event organizers can review materials"
  ON yi_connect.event_materials FOR UPDATE
  TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM yi_connect.events e
...
CREATE POLICY "Users can view reports for completed events"
  ON yi_connect.event_session_reports FOR SELECT
  TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM yi_connect.events e
...
CREATE POLICY "Trainers and organizers can manage session reports"
  ON yi_connect.event_session_reports FOR ALL
  TO authenticated
  USING (
...
CREATE POLICY "Users can view carpool matches for events they RSVP'd to"
  ON yi_connect.event_carpool_matches FOR SELECT
  TO authenticated
  USING (
...
GRANT ALL ON yi_connect.event_trainer_assignments TO authenticated;
GRANT ALL ON yi_connect.event_materials TO authenticated;
GRANT ALL ON yi_connect.event_session_reports TO authenticated;
GRANT ALL ON yi_connect.event_carpool_matches TO authenticated;
