...
export default async function HostAgendaDay2Page() {
  ...
  let sections = await getSections(eventId, 2);

  if (sections.length === 0) {
    await seedNationalSections(eventId);
    sections = await getSections(eventId, 2);
  }
  ...
  async function activate(formData: FormData) {
    ...
    await activateNationalSection(eventId, 2, key);
  }
  ...
  async function saveNotes(formData: FormData) {
    ...
    await updateNationalSectionNotes(eventId, 2, key, notes);
  }
    ...
    <div className="space-y-6">
        ...
        <div className="mt-2 flex items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs text-navy/40">
              Semi-Final · Grand Final · Opportunity Interviews · Recognition
              [HPB §4 Day 2]
            </p>
          </div>
          <div className="flex flex-col items-end gap-2">
            <Link
              href="/yi-future/host/agenda/day1"
      ...
      <section className="bg-white border border-navy/10 rounded-lg p-5">
        <h3 className="text-sm font-bold text-navy mb-4">
          Day 2 sections ({NATIONAL_DAY2_SECTIONS.length})
        </h3>
