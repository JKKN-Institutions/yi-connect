...
import { ArrowLeft, Edit, CheckCircle, XCircle, DollarSign, Calendar, User, FileText, CreditCard, Clock } from 'lucide-react'
...
import { ReimbursementStatusBadge, PaymentMethodBadge } from '@/components/finance/status-badges'
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {paidAt && submittedAt
                ? Math.floor((paidAt.getTime() - submittedAt.getTime()) / (1000 * 60 * 60 * 24))
                : daysPending}
            </div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                {request.requester_profile?.email && (
                  <p className="text-xs text-muted-foreground">{request.requester_profile.email}</p>
                )}
              </div>
            ...
            {request.event && (
              <div>
                <p className="text-sm font-medium text-muted-foreground">Related Event</p>
            ...
            {request.expense && (
              <div>
                <p className="text-sm font-medium text-muted-foreground">Linked Expense</p>
                <Link
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div>
              ...
              <p className="text-2xl font-bold mt-1">{formatCurrency(request.amount)}</p>
            </div>
      ...
      {request.approvals && request.approvals.length > 0 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {request.approvals.length} approval step{request.approvals.length !== 1 ? 's' : ''}
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              {request.approvals.map((approval, index) => {
                const approvalDate = approval.created_at ? new Date(approval.created_at) : null
