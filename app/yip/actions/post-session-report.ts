...
export async function generatePostSessionReport(
  ...
  const [
    ...
    supabase.from("parliamentary_motions").select("motion_type, status, outcome").eq("event_id", eventId),
    supabase.from("bills").select("title, party_side, status, votes_for, votes_against").eq("event_id", eventId),
    supabase.from("participant_fees").select("amount_inr, is_paid").eq("event_id", eventId),
    supabase.from("volunteers").select("station, is_yuva, arrived").eq("event_id", eventId),
    supabase.from("organizer_checklist").select("category, is_completed").eq("event_id", eventId),
