-- ═══════════════════════════════════════════════════════════════════════
-- Migration: Session Bookings lifted to yi_connect.*
-- Phase A Batch 10c
--
-- Source uses UNQUALIFIED table names (CREATE TABLE foo) so we use
-- SET search_path approach. Transformations:
--   - SET search_path TO yi_connect, public, extensions
--   - public.chapters → yi.chapters
--   - public.members / public.events stay implicit (resolved via path)
--   - public.trainer_profiles → trainer_profiles (resolved to yi_connect.*)
--   - set_updated_at() resolves to yi_connect.set_updated_at
--   - SOURCE BUG FIX: View used members.full_name but full_name lives on
--     yi_connect.profiles. Join profiles via members.id = profiles.id.
--
-- Source: 20251128000003_session_bookings.sql (646 lines)
-- Dependencies: yi_connect.trainer_profiles, yi_connect.members,
--               yi_connect.events, yi_connect.profiles, schools, colleges,
--               yi_connect.set_updated_at, yi_connect.get_user_hierarchy_level,
--               yi_connect.user_belongs_to_chapter,
--               yi_connect.get_coordinator_stakeholder_id,
--               yi_connect.is_vertical_chair
-- ═══════════════════════════════════════════════════════════════════════
...
-- ============================================================================
CREATE TABLE IF NOT EXISTS stakeholder_coordinators (
...
CREATE TABLE IF NOT EXISTS session_bookings (
  ...
  coordinator_id UUID NOT NULL REFERENCES stakeholder_coordinators(id) ON DELETE CASCADE,
  ...
  -- Trainer assignment
  event_id UUID REFERENCES events(id) ON DELETE SET NULL, -- Links to created event
  lead_trainer_id UUID REFERENCES trainer_profiles(id),
  trainer_assigned_at TIMESTAMPTZ,
  trainer_assigned_by UUID REFERENCES members(id),
...
-- ============================================================================
CREATE TABLE IF NOT EXISTS session_booking_history (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- ============================================================================
CREATE TABLE IF NOT EXISTS session_booking_trainers (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  booking_id UUID NOT NULL REFERENCES session_bookings(id) ON DELETE CASCADE,
...
-- ============================================================================
CREATE OR REPLACE FUNCTION yi_connect.validate_booking_advance_time()
RETURNS TRIGGER AS $$
DECLARE
...
FOR EACH ROW
EXECUTE FUNCTION yi_connect.validate_booking_advance_time();

COMMENT ON FUNCTION yi_connect.validate_booking_advance_time IS 'Enforces Rule 1: 7-day advance booking for schools/colleges';

-- ============================================================================
-- FUNCTION: evaluate_cancellation_penalty
-- Calculates cancellation penalty based on timing (Rule 1)
-- ============================================================================
CREATE OR REPLACE FUNCTION yi_connect.evaluate_cancellation_penalty(
  p_booking_id UUID,
...
COMMENT ON FUNCTION yi_connect.evaluate_cancellation_penalty IS 'Applies Rule 1 cancellation policy';

-- ============================================================================
-- FUNCTION: track_booking_status_change
-- Auto-records status changes to history
-- ============================================================================
CREATE OR REPLACE FUNCTION yi_connect.track_booking_status_change()
RETURNS TRIGGER AS $$
BEGIN
...
FOR EACH ROW
EXECUTE FUNCTION yi_connect.track_booking_status_change();

-- ============================================================================
-- FUNCTION: count_coordinator_warnings
-- Counts cancellation warnings for a coordinator (for blocking logic)
-- ============================================================================
CREATE OR REPLACE FUNCTION yi_connect.count_coordinator_warnings(p_coordinator_id UUID)
RETURNS INTEGER AS $$
BEGIN
  RETURN (
    SELECT COUNT(*)
...
COMMENT ON FUNCTION yi_connect.count_coordinator_warnings IS 'Counts recent cancellation warnings for a coordinator';
...
CREATE POLICY "coordinators_select_yi_members" ON stakeholder_coordinators
  FOR SELECT USING (
    yi_connect.user_belongs_to_chapter(chapter_id)
  );
...
CREATE POLICY "bookings_select_coordinator" ON session_bookings
  FOR SELECT USING (
    stakeholder_id = yi_connect.get_coordinator_stakeholder_id()
  );
...
CREATE POLICY "bookings_insert_yi_members" ON session_bookings
  FOR INSERT WITH CHECK (
    yi_connect.user_belongs_to_chapter(chapter_id)
    AND yi_connect.get_user_hierarchy_level() >= 2
  );
...
CREATE POLICY "booking_history_select" ON session_booking_history
  FOR SELECT USING (
    booking_id IN (
      ...
      WHERE yi_connect.user_belongs_to_chapter(chapter_id)
      OR stakeholder_id = yi_connect.get_coordinator_stakeholder_id()
    )
  );
...
CREATE POLICY "booking_trainers_insert" ON session_booking_trainers
  FOR INSERT WITH CHECK (
    yi_connect.get_user_hierarchy_level() >= 2
    OR yi_connect.is_vertical_chair()
  );
...
-- ============================================================================
-- NOTE: SOURCE BUG FIX. Source used m.full_name from members but full_name
-- is on yi_connect.profiles (members extend profiles via id FK).
-- We JOIN profiles directly via tp.member_id (which equals profiles.id).
...
SELECT
  ...
  yi_connect.count_coordinator_warnings(sb.coordinator_id) AS warning_count
