...
import { getStakeholderOverview, getPendingFollowUps, getExpiringMous } from '@/lib/data/stakeholder'
import { formatStakeholderType } from '@/types/stakeholder'
import { redirect } from 'next/navigation'

export const metadata = {
  title: 'Stakeholder CRM',
  description: 'Manage relationships with schools, colleges, industries, government, NGOs, vendors, and speakers',
}
    ...
    <>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{stats.interactions_this_month}</div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        {stakeholderTypes.map((stakeholder) => {
          ...
          const count = stats.by_type[stakeholder.type as keyof typeof stats.by_type]

          return (
            <Card key={stakeholder.type} className="hover:shadow-md transition-shadow">
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">{stakeholder.label}</CardTitle>
          ...
          <div className="grid gap-4 md:grid-cols-3">
            <div className="flex items-center gap-4">
              <div className="flex-1">
                ...
                <p className="text-2xl font-bold">{stats.health_distribution.healthy}</p>
              </div>
            ...
            <div className="flex items-center gap-4">
              <div className="flex-1">
                ...
                <p className="text-2xl font-bold">{stats.health_distribution.needs_attention}</p>
              </div>
            ...
            <div className="flex items-center gap-4">
              <div className="flex-1">
                ...
                <p className="text-2xl font-bold">{stats.health_distribution.at_risk}</p>
              </div>
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          {followUps.length} interaction{followUps.length !== 1 ? 's' : ''} require follow-up
        </CardDescription>
      </CardHeader>
      <CardContent>
                ...
                <div className="flex items-center gap-2">
                  {followUpDate && (
                    <Badge variant={isOverdue ? 'destructive' : 'secondary'} className="text-xs">
                      {isOverdue ? 'Overdue' : followUpDate.toLocaleDateString()}
                    </Badge>
                  )}
                </div>
              </div>
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          {expiringMous.length} MoU{expiringMous.length !== 1 ? 's' : ''} expiring in the next 30 days
        </CardDescription>
      </CardHeader>
      <CardContent>
        <div className="space-y-3">
