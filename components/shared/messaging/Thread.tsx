...
/**
 * Generic send action signature.
 * Yi Future passes its own `sendMessage(threadId, body)`.
 * Yi Connect (or any future consumer) passes its own action too.
 */
type SendAction = (
  threadId: string,
  body: string
) => Promise<{ ok: boolean; data?: Message; error?: string }>;

type Props = {
  threadId: string;
  currentSender: CurrentSender;
  initialMessages: Message[];
  /** Optional custom send action. Defaults to Yi Future's sendMessage. */
...
export function Thread({ threadId, currentSender, initialMessages, sendAction }: Props) {
  ...
  // Lazily import Yi Future's sendMessage only when no sendAction provided
  const sendActionRef = useRef<SendAction | null>(sendAction ?? null);
  useEffect(() => {
    if (sendAction) {
      sendActionRef.current = sendAction;
      return;
    }
    // Dynamic import to avoid hard coupling when used outside Yi Future
    import("@/app/yi-future/actions/messages").then((mod) => {
      sendActionRef.current = mod.sendMessage as unknown as SendAction;
    });
  }, [sendAction]);
  ...
  const handleSend = useCallback(
    async (raw: string) => {
      ...
      const action = sendActionRef.current;
      if (!action) {
        setError("Send not available yet. Try again.");
        return;
      }
      ...
      const res = await action(threadId, body);
