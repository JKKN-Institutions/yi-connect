...
async function getDefaultRubric(
  editionId: string
...
export default async function PrelimScoringPage() {
  ...
  // Group evals by team, only count submitted ones
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h2 className="text-2xl font-bold text-navy">Prelim Scoring</h2>
        <p className="mt-1 text-sm text-navy/60">
          {ctx.chapterName} &middot; Evaluations averaged across jurors.
          Threshold {threshold}/{totalMax} to advance.
        </p>
      </div>

      {!rubric && (
        <div className="bg-yi-saffron/10 border border-yi-saffron/30 rounded-md p-3 text-sm text-yi-saffron">
          No default chapter rubric found. Scores will display without
          threshold checks until a rubric is configured.
        </div>
      )}
      ...
      <p className="text-xs text-navy/40">
        <Link
          href="/yi-future/host/prelim/teams"
