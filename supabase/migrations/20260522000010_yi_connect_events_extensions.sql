-- ═══════════════════════════════════════════════════════════════════════
-- Migration: Industry Opportunities (Batch 7.5, revised)
--
-- NOTE: events_trainer_materials was originally bundled here but it
-- references public.trainer_profiles which is NEVER CREATED in any
-- yi-connect migration in this repo. Skipped — deferred until we
-- locate or create the missing trainer_profiles schema.
--
-- Lifts: 20251127000002 industry_opportunities
-- ═══════════════════════════════════════════════════════════════════════
...
-- Opportunity Type
CREATE TYPE yi_connect.opportunity_type AS ENUM (
  'industrial_visit',
  'internship',
  'mentorship',
  'guest_lecture',
...
-- Opportunity Status
CREATE TYPE yi_connect.opportunity_status AS ENUM (
  'draft',
  'published',
...
-- Application Status
CREATE TYPE yi_connect.application_status AS ENUM (
  'draft',
  'pending_review',
  'under_review',
  'shortlisted',
...
CREATE TYPE yi_connect.visit_request_status AS ENUM (
  'pending_yi_review',
  'yi_approved',
...
-- Visit Type
CREATE TYPE yi_connect.visit_type AS ENUM (
  'solo',
  'group'
);

-- Partnership Stage (for MoU lifecycle)
CREATE TYPE yi_connect.partnership_stage AS ENUM (
  'initial_contact',
  'negotiation',
...
CREATE TABLE IF NOT EXISTS yi_connect.industry_opportunities (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  chapter_id UUID NOT NULL REFERENCES yi.chapters(id) ON DELETE CASCADE,
  industry_id UUID NOT NULL REFERENCES yi_connect.industries(id) ON DELETE CASCADE,

  -- Basic Information
  title TEXT NOT NULL,
  description TEXT NOT NULL,
  opportunity_type yi_connect.opportunity_type NOT NULL,
  status yi_connect.opportunity_status NOT NULL DEFAULT 'draft',
  ...
  -- Full-text search
  search_vector tsvector  -- generation disabled
);

-- Indexes
CREATE INDEX idx_opportunities_chapter ON yi_connect.industry_opportunities(chapter_id);
CREATE INDEX idx_opportunities_industry ON yi_connect.industry_opportunities(industry_id);
CREATE INDEX idx_opportunities_status ON yi_connect.industry_opportunities(status);
CREATE INDEX idx_opportunities_type ON yi_connect.industry_opportunities(opportunity_type);
CREATE INDEX idx_opportunities_deadline ON yi_connect.industry_opportunities(application_deadline)
  WHERE status IN ('published', 'accepting_applications');
CREATE INDEX idx_opportunities_featured ON yi_connect.industry_opportunities(is_featured, published_at DESC)
  WHERE status = 'published';
CREATE INDEX idx_opportunities_search ON yi_connect.industry_opportunities USING gin(search_vector);
CREATE INDEX idx_opportunities_eligibility ON yi_connect.industry_opportunities USING gin(eligibility_criteria);

COMMENT ON TABLE yi_connect.industry_opportunities IS 'Opportunities posted by industries for Yi members';

-- ============================================================================
-- OPPORTUNITY APPLICATIONS TABLE
-- ============================================================================

CREATE TABLE IF NOT EXISTS yi_connect.opportunity_applications (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  opportunity_id UUID NOT NULL REFERENCES yi_connect.industry_opportunities(id) ON DELETE CASCADE,
  ...
  -- Status Workflow
  status yi_connect.application_status NOT NULL DEFAULT 'draft',
...
-- Indexes
CREATE INDEX idx_applications_opportunity ON yi_connect.opportunity_applications(opportunity_id);
CREATE INDEX idx_applications_member ON yi_connect.opportunity_applications(member_id);
CREATE INDEX idx_applications_status ON yi_connect.opportunity_applications(status);
CREATE INDEX idx_applications_match_score ON yi_connect.opportunity_applications(match_score DESC);
CREATE INDEX idx_applications_submitted ON yi_connect.opportunity_applications(applied_at DESC)
  WHERE status != 'draft';
CREATE INDEX idx_applications_pending ON yi_connect.opportunity_applications(opportunity_id, status)
  WHERE status = 'pending_review';

COMMENT ON TABLE yi_connect.opportunity_applications IS 'Member applications for industry opportunities with match scoring';

-- ============================================================================
-- MEMBER VISIT REQUESTS TABLE
-- ============================================================================

CREATE TABLE IF NOT EXISTS yi_connect.member_visit_requests (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  chapter_id UUID NOT NULL REFERENCES yi.chapters(id) ON DELETE CASCADE,

  -- Requestor
  requested_by UUID NOT NULL REFERENCES yi_connect.members(id),

  -- Target Industry (must have active MoU)
  industry_id UUID NOT NULL REFERENCES yi_connect.industries(id),
  mou_id UUID REFERENCES yi_connect.stakeholder_mous(id), -- Link to active MoU

  -- Request Details
  request_title TEXT NOT NULL,
  visit_purpose TEXT NOT NULL,
  visit_type yi_connect.visit_type NOT NULL DEFAULT 'solo',
  ...
  -- Status Workflow
  status yi_connect.visit_request_status NOT NULL DEFAULT 'pending_yi_review',

  -- Yi Review (Industry Chair)
  yi_reviewer_id UUID REFERENCES yi_connect.members(id),
  ...
  industry_contacted_by UUID REFERENCES yi_connect.members(id),
  ...
  converted_event_id UUID REFERENCES yi_connect.events(id),
...
-- Indexes
CREATE INDEX idx_visit_requests_chapter ON yi_connect.member_visit_requests(chapter_id);
CREATE INDEX idx_visit_requests_member ON yi_connect.member_visit_requests(requested_by);
CREATE INDEX idx_visit_requests_industry ON yi_connect.member_visit_requests(industry_id);
CREATE INDEX idx_visit_requests_status ON yi_connect.member_visit_requests(status);
CREATE INDEX idx_visit_requests_pending ON yi_connect.member_visit_requests(created_at DESC)
  WHERE status = 'pending_yi_review';
CREATE INDEX idx_visit_requests_scheduled ON yi_connect.member_visit_requests(scheduled_date)
  WHERE status = 'scheduled';

COMMENT ON TABLE yi_connect.member_visit_requests IS 'Member-initiated visit requests to industry partners (requires active MoU)';

-- ============================================================================
-- VISIT REQUEST INTERESTS TABLE (other members showing interest)
-- ============================================================================

CREATE TABLE IF NOT EXISTS yi_connect.visit_request_interests (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  visit_request_id UUID NOT NULL REFERENCES yi_connect.member_visit_requests(id) ON DELETE CASCADE,
  member_id UUID NOT NULL REFERENCES yi_connect.members(id) ON DELETE CASCADE,
...
CREATE INDEX idx_visit_interests_request ON yi_connect.visit_request_interests(visit_request_id);
CREATE INDEX idx_visit_interests_member ON yi_connect.visit_request_interests(member_id);

-- ============================================================================
-- OPPORTUNITY BOOKMARKS TABLE
-- ============================================================================

CREATE TABLE IF NOT EXISTS yi_connect.opportunity_bookmarks (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  opportunity_id UUID NOT NULL REFERENCES yi_connect.industry_opportunities(id) ON DELETE CASCADE,
  member_id UUID NOT NULL REFERENCES yi_connect.members(id) ON DELETE CASCADE,
...
CREATE INDEX idx_bookmarks_member ON yi_connect.opportunity_bookmarks(member_id);
CREATE INDEX idx_bookmarks_opportunity ON yi_connect.opportunity_bookmarks(opportunity_id);

-- ============================================================================
-- INDUSTRY IMPACT METRICS TABLE
-- ============================================================================

CREATE TABLE IF NOT EXISTS yi_connect.industry_impact_metrics (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  industry_id UUID NOT NULL REFERENCES yi_connect.industries(id) ON DELETE CASCADE UNIQUE,
...
CREATE INDEX idx_industry_impact_industry ON yi_connect.industry_impact_metrics(industry_id);
CREATE INDEX idx_industry_impact_chapter ON yi_connect.industry_impact_metrics(chapter_id);
CREATE INDEX idx_industry_impact_score ON yi_connect.industry_impact_metrics(engagement_score DESC);
CREATE INDEX idx_industry_impact_tier ON yi_connect.industry_impact_metrics(engagement_tier);

COMMENT ON TABLE yi_connect.industry_impact_metrics IS 'Aggregated impact metrics per industry partner';

-- ============================================================================
-- EXTEND STAKEHOLDER_MOUS FOR PARTNERSHIP LIFECYCLE
-- ============================================================================

ALTER TABLE yi_connect.stakeholder_mous
ADD COLUMN IF NOT EXISTS partnership_stage yi_connect.partnership_stage DEFAULT 'initial_contact',
...
INSERT INTO yi_connect.roles (name, description, hierarchy_level)
SELECT 'Industry Coordinator', 'External industry partner coordinator who can post opportunities and review applications', 1
WHERE NOT EXISTS (SELECT 1 FROM yi_connect.roles WHERE name = 'Industry Coordinator');

-- ============================================================================
-- MATCH SCORE CALCULATION FUNCTION
-- ============================================================================

CREATE OR REPLACE FUNCTION yi_connect.calculate_opportunity_match_score(
  p_member_id UUID,
...
BEGIN
  ...
  FROM yi_connect.industry_opportunities
...
COMMENT ON FUNCTION yi_connect.calculate_opportunity_match_score IS 'Calculate member-opportunity match score (100 points max)';
...
BEGIN
  ...
  UPDATE yi_connect.industry_opportunities
  SET
    current_applications = (
      SELECT COUNT(*) FROM yi_connect.opportunity_applications
...
DROP TRIGGER IF EXISTS trigger_update_application_count ON yi_connect.opportunity_applications;
CREATE TRIGGER trigger_update_application_count
  AFTER INSERT OR UPDATE OR DELETE ON yi_connect.opportunity_applications
...
RETURNS TRIGGER AS $$
BEGIN
  UPDATE yi_connect.member_visit_requests
  SET
    interest_count = (
      SELECT COUNT(*) + 1 FROM yi_connect.visit_request_interests
...
DROP TRIGGER IF EXISTS trigger_update_interest_count ON yi_connect.visit_request_interests;
CREATE TRIGGER trigger_update_interest_count
  AFTER INSERT OR DELETE ON yi_connect.visit_request_interests
...
BEGIN
  ...
  INSERT INTO yi_connect.industry_impact_metrics (industry_id, chapter_id)
  SELECT NEW.industry_id, NEW.chapter_id
  WHERE NOT EXISTS (
    SELECT 1 FROM yi_connect.industry_impact_metrics WHERE industry_id = NEW.industry_id
  );

  -- Update metrics
  UPDATE yi_connect.industry_impact_metrics
  SET
    total_opportunities_posted = (
      SELECT COUNT(*) FROM yi_connect.industry_opportunities
...
DROP TRIGGER IF EXISTS trigger_update_industry_impact ON yi_connect.industry_opportunities;
CREATE TRIGGER trigger_update_industry_impact
  AFTER INSERT OR UPDATE ON yi_connect.industry_opportunities
...
DROP TRIGGER IF EXISTS update_industry_opportunities_updated_at ON yi_connect.industry_opportunities;
CREATE TRIGGER update_industry_opportunities_updated_at
  BEFORE UPDATE ON yi_connect.industry_opportunities
  FOR EACH ROW EXECUTE FUNCTION yi_connect.update_updated_at_column();

DROP TRIGGER IF EXISTS update_opportunity_applications_updated_at ON yi_connect.opportunity_applications;
CREATE TRIGGER update_opportunity_applications_updated_at
  BEFORE UPDATE ON yi_connect.opportunity_applications
  FOR EACH ROW EXECUTE FUNCTION yi_connect.update_updated_at_column();

DROP TRIGGER IF EXISTS update_member_visit_requests_updated_at ON yi_connect.member_visit_requests;
CREATE TRIGGER update_member_visit_requests_updated_at
  BEFORE UPDATE ON yi_connect.member_visit_requests
  FOR EACH ROW EXECUTE FUNCTION yi_connect.update_updated_at_column();

DROP TRIGGER IF EXISTS update_industry_impact_metrics_updated_at ON yi_connect.industry_impact_metrics;
CREATE TRIGGER update_industry_impact_metrics_updated_at
  BEFORE UPDATE ON yi_connect.industry_impact_metrics
  FOR EACH ROW EXECUTE FUNCTION yi_connect.update_updated_at_column();
...
ALTER TABLE yi_connect.industry_opportunities ENABLE ROW LEVEL SECURITY;
ALTER TABLE yi_connect.opportunity_applications ENABLE ROW LEVEL SECURITY;
ALTER TABLE yi_connect.member_visit_requests ENABLE ROW LEVEL SECURITY;
ALTER TABLE yi_connect.visit_request_interests ENABLE ROW LEVEL SECURITY;
ALTER TABLE yi_connect.opportunity_bookmarks ENABLE ROW LEVEL SECURITY;
ALTER TABLE yi_connect.industry_impact_metrics ENABLE ROW LEVEL SECURITY;

-- Industry Opportunities Policies
CREATE POLICY "Members can view published opportunities"
  ON yi_connect.industry_opportunities FOR SELECT
  TO authenticated
...
CREATE POLICY "Industry coordinators can manage their opportunities"
  ON yi_connect.industry_opportunities FOR ALL
  TO authenticated
  USING (
...
CREATE POLICY "Members can view their own applications"
  ON yi_connect.opportunity_applications FOR SELECT
  TO authenticated
  USING (member_id = auth.uid());

CREATE POLICY "Members can create their own applications"
  ON yi_connect.opportunity_applications FOR INSERT
  TO authenticated
  WITH CHECK (member_id = auth.uid());

CREATE POLICY "Members can update their own draft applications"
  ON yi_connect.opportunity_applications FOR UPDATE
  TO authenticated
...
CREATE POLICY "Industry coordinators can view applications for their opportunities"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM yi_connect.industry_opportunities io
...
CREATE POLICY "Members can view their own visit requests"
  ON yi_connect.member_visit_requests FOR SELECT
  TO authenticated
  USING (requested_by = auth.uid());

CREATE POLICY "Members can create visit requests"
  ON yi_connect.member_visit_requests FOR INSERT
  TO authenticated
  WITH CHECK (requested_by = auth.uid());

CREATE POLICY "Yi chairs can manage visit requests"
  ON yi_connect.member_visit_requests FOR ALL
  TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM yi_connect.user_roles ur
...
CREATE POLICY "Members can view interests for visible requests"
  ON yi_connect.visit_request_interests FOR SELECT
  TO authenticated
  USING (
    member_id = auth.uid() OR
    EXISTS (
      SELECT 1 FROM yi_connect.member_visit_requests vr
...
GRANT ALL ON yi_connect.industry_opportunities TO authenticated;
GRANT ALL ON yi_connect.opportunity_applications TO authenticated;
GRANT ALL ON yi_connect.member_visit_requests TO authenticated;
GRANT ALL ON yi_connect.visit_request_interests TO authenticated;
GRANT ALL ON yi_connect.opportunity_bookmarks TO authenticated;
GRANT ALL ON yi_connect.industry_impact_metrics TO authenticated;
