...
export async function getMockDataStats(): Promise<MockDataStats> {
  ...
  const [
    ...
    supabase.from("seasons").select("id", { count: "exact", head: true }).eq("is_mock", true),
    ...
    supabase.from("people").select("id", { count: "exact", head: true }).eq("is_mock", true),
    ...
    supabase.from("parliamentary_motions").select("id", { count: "exact", head: true }).eq("is_mock", true),
    ...
    supabase.from("participant_fees").select("id", { count: "exact", head: true }).eq("is_mock", true),
    supabase.from("volunteers").select("id", { count: "exact", head: true }).eq("is_mock", true),
    supabase.from("feedback_responses").select("id", { count: "exact", head: true }).eq("is_mock", true),
    supabase.from("event_media").select("id", { count: "exact", head: true }).eq("is_mock", true),
    supabase.from("branding_compliance_checks").select("id", { count: "exact", head: true }).eq("is_mock", true),
    supabase.from("invitation_approvals").select("id", { count: "exact", head: true }).eq("is_mock", true),
    ...
    supabase.from("organizer_profiles").select("id", { count: "exact", head: true }).eq("is_mock", true),
  ]);
...
export async function seedMockData(): Promise<ActionResult<MockDataStats>> {
  ...
  try {
    ...
    const people: Array<{ id: string; full_name: string; phone: string; school_idx: number }> =
      [];
    const peopleInserts: Array<WithMock<Database["public"]["Tables"]["people"]["Insert"]>> =
      [];
    for (let i = 0; i < 30; i++) {
      const isMale = i % 2 === 0;
      const name = isMale
    ...
    const participantInserts: Array<
      WithMock<Database["public"]["Tables"]["participants"]["Insert"]>
    > = [];
    for (let i = 0; i < 30; i++) {
      const p = plan[i]!;
    ...
    const scoreInserts: Array<WithMock<Database["public"]["Tables"]["scores"]["Insert"]>> =
      [];
    ...
    const resultInserts: Array<Database["public"]["Tables"]["results"]["Insert"]> =
    ...
    const motionInserts: Array<
      WithMock<Database["public"]["Tables"]["parliamentary_motions"]["Insert"]>
    > = [
      {
        event_id: chapterEvent.id,
    ...
    await supabase.from("parliamentary_motions").insert(motionInserts);
    ...
    const billInserts: Array<WithMock<Database["public"]["Tables"]["bills"]["Insert"]>> = [
      {
        event_id: chapterEvent.id,
    ...
    const questionInserts: Array<
      WithMock<Database["public"]["Tables"]["questions"]["Insert"]>
    > = [];
    for (let i = 0; i < 10; i++) {
    ...
    const feeInserts: Array<
      WithMock<Database["public"]["Tables"]["participant_fees"]["Insert"]>
    ...
    await supabase.from("participant_fees").insert(feeInserts);
    ...
    const volInserts: Array<
      WithMock<Database["public"]["Tables"]["volunteers"]["Insert"]>
    ...
    const feedbackInserts: Array<
      WithMock<Database["public"]["Tables"]["feedback_responses"]["Insert"]>
    > = [];
    // 10 participants
    for (let i = 0; i < 10; i++) {
    ...
    await supabase.from("feedback_responses").insert(feedbackInserts);
    ...
    const mediaInserts: Array<
      WithMock<Database["public"]["Tables"]["event_media"]["Insert"]>
    ...
    await supabase.from("event_media").insert(mediaInserts);

    // 19. Branding compliance checks (3) ────────────────────────────────
    await supabase.from("branding_compliance_checks").insert([
      {
        event_id: chapterEvent.id,
    ...
    await supabase.from("invitation_approvals").insert({
      event_id: chapterEvent.id,
    ...
    const registrationInserts: Array<
      WithMock<Database["public"]["Tables"]["registrations"]["Insert"]>
    > = participantsRows.map((p, idx) => ({
      event_id: chapterEvent.id,
      participant_id: p.id,
    ...
    if (regionalEvent && sortedParticipants.length > 0) {
      ...
      if (rParts && rubric) {
        ...
        const regionalScoreInserts: Array<
          WithMock<Database["public"]["Tables"]["scores"]["Insert"]>
    ...
    const checklistInserts: Array<
      Database["public"]["Tables"]["organizer_checklist"]["Insert"]
    > = [
      {
        event_id: chapterEvent.id,
    ...
    await supabase.from("organizer_checklist").insert(checklistInserts);
    ...
    const { data: centralTopic } = await supabase
      ...
      .maybeSingle();
    if (centralTopic) {
      await supabase.from("event_topic_assignments").insert({
        event_id: chapterEvent.id,
...
> {
  ...
  try {
    ...
    if (scoreIds.length > 0) {
      await wipe("score_audit_log", async () =>
        supabase
    ...
    if (mockEventIds.length > 0) {
      ...
      await wipe("organizer_checklist", async () =>
        supabase
          .from("organizer_checklist")
          .delete({ count: "exact" })
          .in("event_id", mockEventIds)
      );
      await wipe("event_topic_assignments", async () =>
        supabase
          .from("event_topic_assignments")
          .delete({ count: "exact" })
          .in("event_id", mockEventIds)
    ...
    await wipe("parliamentary_motions", async () =>
      supabase
    ...
    await wipe("participant_fees", async () =>
      supabase
        .from("participant_fees")
        .delete({ count: "exact" })
        .eq("is_mock", true)
    );
    ...
    await wipe("feedback_responses", async () =>
      supabase
        .from("feedback_responses")
        .delete({ count: "exact" })
        .eq("is_mock", true)
    );
    await wipe("event_media", async () =>
      supabase
        .from("event_media")
        .delete({ count: "exact" })
        .eq("is_mock", true)
    ...
    await wipe("invitation_approvals", async () =>
      supabase
        .from("invitation_approvals")
        .delete({ count: "exact" })
        .eq("is_mock", true)
    );
    await wipe("branding_compliance_checks", async () =>
      supabase
        .from("branding_compliance_checks")
        .delete({ count: "exact" })
        .eq("is_mock", true)
    );
    ...
    await wipe("people", async () =>
      supabase.from("people").delete({ count: "exact" }).eq("is_mock", true)
    );
    void mockPersonIds;
    await wipe("organizer_profiles", async () =>
      supabase
        .from("organizer_profiles")
        .delete({ count: "exact" })
        .eq("is_mock", true)
    );
    await wipe("seasons", async () =>
      supabase.from("seasons").delete({ count: "exact" }).eq("is_mock", true)
    );
