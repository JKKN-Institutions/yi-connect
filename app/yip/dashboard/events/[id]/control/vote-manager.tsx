...
export function VoteManager({
  ...
  // Parallel party-leader elections may be live on ANY agenda item (incl. a
  // non-voting one). Keep the panel mounted whenever any session is active so
  // the per-party Close/Reveal list stays reachable.

  if (!showVoteControls && !voteSession && activeVoteSessions.length === 0)
    return null;
