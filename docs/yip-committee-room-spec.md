...
**Status:** SIGNED OFF — building. **Decisions locked (2026-06-27):** spec-first; bill edited by
**chair + lead drafter**; **committee-internal amendment voting included**. **§9 resolved:**
(1) no-chair fallback = **lead_drafter, else organiser-only + assign-chair prompt**;
(2) amendment carry = **chair/lead decides, vote tally shown as advice**;
(3) readiness min provisions = **≥3**; (4) `/yip/me/bill` = **redirect into `/yip/me/committee`**.
...
## 9. Open questions — RESOLVED (2026-06-27, director sign-off)
- **Chair fallback:** ✅ **lead_drafter edits; if neither chair nor lead_drafter is set,
  organiser-only + a visible "assign a chair" prompt.** (Not auto-promote.)
- **Amendment carry rule:** ✅ **chair/lead drafter decides accept/reject; the for/against tally
  is shown as advice only** (no auto-carry on majority).
- **Min provisions for readiness:** ✅ **≥3 provisions** required before Submit unlocks.
- **`/yip/me/bill`:** ✅ **redirect into `/yip/me/committee`** — single source, no duplicate
  surface. (`/yip/me/ministry` likewise folds in.)
