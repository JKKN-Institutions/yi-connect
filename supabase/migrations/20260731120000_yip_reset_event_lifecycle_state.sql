...
--   RESET lifecycle (added 2026-07-31): events.status -> 'draft',
--     events.current_agenda_item_id -> null, and every agenda row back to
--     'upcoming' with its run timestamps / skip reason cleared.
...
begin
  ...
  -- Lifecycle reset. Without this the event keeps whatever status the
  -- rehearsal left it on -- "completed" being the common case -- and the
  -- Control panel offers NO forward transition out of 'completed', stranding
  -- the chair with a wiped event that still reads as finished. 'draft'
  -- restores the "Start Day 1" button; joining by access code does not gate
  -- on status, so students are unaffected.
  update agenda set
    status = 'upcoming',
    actual_start = null,
    actual_end = null,
    skip_reason = null
  where event_id = p_event_id;
  get diagnostics n = row_count; v := v || jsonb_build_object('agenda_reset', n);

  update events set
    status = 'draft',
    current_agenda_item_id = null
  where id = p_event_id;
  get diagnostics n = row_count; v := v || jsonb_build_object('event_status_reset', n);
