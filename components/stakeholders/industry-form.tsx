...
import type { IndustryFormInput } from '@/types/stakeholder'
  ...
  const onSubmit = async (data: IndustryFormInput) => {
    startTransition(async () => {
      try {
        ...
        if (data.annual_turnover) formData.append('annual_turnover', data.annual_turnover)

        // CSR & Sponsorship
        formData.append('has_csr_program', String(data.has_csr_program || false))
        if (data.csr_budget_range) formData.append('csr_budget_range', data.csr_budget_range)
        if (data.csr_focus_areas && data.csr_focus_areas.length > 0) {
          formData.append('csr_focus_areas', JSON.stringify(data.csr_focus_areas))
        }
        if (data.sponsorship_potential)
          formData.append('sponsorship_potential', data.sponsorship_potential)

        // Collaboration
        if (data.collaboration_interests && data.collaboration_interests.length > 0) {
          formData.append('collaboration_interests', JSON.stringify(data.collaboration_interests))
        }
        if (data.available_resources && data.available_resources.length > 0) {
          formData.append('available_resources', JSON.stringify(data.available_resources))
        }
        formData.append('can_provide_internships', String(data.can_provide_internships || false))
        formData.append('can_provide_mentorship', String(data.can_provide_mentorship || false))
        ...
        if (result.success) {
          ...
          toast.error(result.error || 'Failed to create industry')
        }
      } catch (error) {
  ...
  const addCollaborationInterest = () => {
    if (collaborationInterest.trim()) {
      ...
      if (!current.includes(collaborationInterest.trim())) {
        form.setValue('collaboration_interests', [...current, collaborationInterest.trim()])
  ...
  const addAvailableResource = () => {
    if (availableResource.trim()) {
      ...
      if (!current.includes(availableResource.trim())) {
        form.setValue('available_resources', [...current, availableResource.trim()])
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
        ...
        <div className="space-y-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Organization location details</p>
          </div>
            ...
            render={({ field }) => (
              <FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4">
                ...
                <div className="space-y-1 leading-none">
                  ...
                  <FormDescription>
                    Check if this organization has a Corporate Social Responsibility program
                  </FormDescription>
                </div>
              </FormItem>
            )}
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <div className="flex gap-2">
                  ...
                  />
                  <Button type="button" onClick={addCsrFocusArea} variant="secondary">
                    Add
                  </Button>
                </div>
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <div className="flex gap-2">
                  ...
                  />
                  <Button type="button" onClick={addCollaborationInterest} variant="secondary">
                    Add
                  </Button>
                </div>
                {field.value && field.value.length > 0 && (
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <div className="flex gap-2">
                  ...
                  />
                  <Button type="button" onClick={addAvailableResource} variant="secondary">
                    Add
                  </Button>
                </div>
                {field.value && field.value.length > 0 && (
        ...
        <div className="space-y-4">
          <div>
            <h3 className="text-lg font-medium">Connection & Operational Details</h3>
            <p className="text-sm text-muted-foreground">
